...
import '/components/background_admin.dart';
...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<List<Order>> _fetchOrders() async {
    ...
    final String action = prefs.getString("Authorization");
    final response =
        await http.get(Uri.parse('http://192.168.56.1:8000/orders/'), headers: {
      "Authorization": 'Token $action',
    });
  ...
  Future<int> _fetchProductsCount() async {
    ...
    final response = await http.get(
        Uri.parse('http://192.168.56.1:8000/count-product/'),
  ...
  Future<int> _fetchVisitorsCount() async {
    ...
    final String action = prefs.getString("Authorization");
    final response = await http.get(
        Uri.parse('http://192.168.56.1:8000/count-users/'),
        headers: {"Authorization": 'Token $action'});
    if (response.statusCode == 200) {
      final data = json.decode(response.body) as int;
      return data;
  ...
  Future<int> _fetchOrdersCount() async {
    ...
    final String action = prefs.getString("Authorization");
    final response = await http.get(
        Uri.parse('http://192.168.56.1:8000/count-orders/'),
        headers: {"Authorization": 'Token $action'});
    if (response.statusCode == 200) {
      final data = json.decode(response.body) as int;
      return data;
